...
public class DN06_63160303 {
				...
				if (plevel[vrstica][stolpec] == 1) {
					...
					while ((tmpVrstica) < stVrstic && (plevel[tmpVrstica][tmpStolpec] == 1)) {
						...
						if (izracunNaLevo < najboljLevo) {
							int razlika = najboljLevo - izracunNaLevo;
							najboljLevo = izracunNaLevo;
							sirina += razlika;
						}

						// uporabil razlika zgoraj
						/*if (kolikoDesno[tmpVrstica][izracunNaLevo] > sirina) {
							sirina = kolikoDesno[tmpVrstica][izracunNaLevo];
						}*/
						if ((kolikoLevo[tmpVrstica][najboljLevo]) > 1) {
							int zamik = (kolikoLevo[tmpVrstica][najboljLevo] - 1);
							System.out.println("levejse: " + zamik);
							najboljLevo -= zamik;
							sirina += zamik;
						}

						if ((kolikoDesno[tmpVrstica][najboljLevo + sirina - 1]) > 1) {
							sirina += (kolikoDesno[tmpVrstica][najboljLevo + sirina - 1]) - 1;
						}


						int najnizje = vrstica + visina - 1;
						int najboljDesno = najboljLevo + sirina - 1;
						int premikanjeDesno = najboljDesno + 1;
						int premikanjeNavzdol = najnizje + 1;
						

						while (premikanjeDesno < stStolpcev) {
							if (plevel[najnizje][premikanjeDesno] == 1) {
								int desnoNajnizje = najnizje;
								while (desnoNajnizje < stVrstic && plevel[desnoNajnizje][premikanjeDesno] == 1) {
					...
					System.out.println("vi: " + visina + " si: " + sirina + " nl: " + najboljLevo + " vr: " + vrstica + "\n");
	...
	public static int jePrazenStolpec() {
		return 0;
	}
